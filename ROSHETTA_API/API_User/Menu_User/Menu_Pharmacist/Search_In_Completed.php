    ...
    if (isset($_SESSION['pharmacist']) && isset($_SESSION['pharmacy'])) {
        ...
        if (isset($_GET['search']) && !empty($_GET['search'])) {
            ...
            $get_prescript = $database->prepare("SELECT prescript.id as prescript_id,prescript.ser_id as prescript_ser_id,date_pay,patient.name as patient_name FROM prescript,patient,pharmacy_prescript,pharmacy 
                                                        WHERE prescript.patient_id = patient.id AND pharmacy.id = :pharmacy_id AND pharmacy_prescript.pharmacy_id = pharmacy.id AND pharmacy_prescript.prescript_id = prescript.id AND (patient.name = :search OR patient.ssd = :search OR prescript.ser_id = :search)  ORDER BY date_pay DESC ");
