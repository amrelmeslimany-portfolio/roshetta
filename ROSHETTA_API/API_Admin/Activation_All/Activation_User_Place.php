...
if (isset($_SESSION['admin'])) {
    ...
    if (isset($_POST['search']) && !empty($_POST['search'])) {
        ...
        $get_not_active_doctor = $database->prepare("SELECT doctor.id as doctor_id , doctor_name , ssd as doctor_ssd , profile_img , front_nationtional_card , back_nationtional_card , graduation_cer , card_id_img FROM doctor,activation_person WHERE doctor.id = activation_person.doctor_id AND activation_person.isactive = 0 AND (doctor.ssd = :search OR doctor_name = :search OR email = :search)");
        ...
        $get_active_clinic = $database->prepare("SELECT clinic.id as clinic_id , clinic_name , clinic.ser_id as clinic_ser_id , clinic.logo , license_img FROM clinic,activation_place WHERE clinic.id = activation_place.clinic_id AND activation_place.isactive = 1 AND (clinic.ser_id = :search OR clinic_name = :search OR clinic.owner = :search)");
        ...
        $get_not_active_clinic = $database->prepare("SELECT clinic.id as clinic_id , clinic_name , clinic.ser_id as clinic_ser_id , clinic.logo , license_img FROM clinic,activation_place WHERE clinic.id = activation_place.clinic_id AND activation_place.isactive = 0 AND (clinic.ser_id = :search OR clinic_name = :search OR clinic.owner = :search)");
        ...
        $get_active_pharmacy = $database->prepare("SELECT pharmacy.id as pharmacy_id , pharmacy_name , pharmacy.ser_id as pharmacy_ser_id , pharmacy.logo , license_img FROM pharmacy,activation_place WHERE pharmacy.id = activation_place.pharmacy_id AND activation_place.isactive = 1 AND (pharmacy.ser_id = :search OR pharmacy_name = :search OR pharmacy.owner = :search)");
        ...
        $get_not_active_pharmacy = $database->prepare("SELECT pharmacy.id as pharmacy_id , pharmacy_name , pharmacy.ser_id as pharmacy_ser_id , pharmacy.logo , license_img FROM pharmacy,activation_place WHERE pharmacy.id = activation_place.pharmacy_id AND activation_place.isactive = 0 AND (pharmacy.ser_id = :search OR pharmacy_name = :search OR pharmacy.owner = :search)");
        ...
        $get_active_doctor = $database->prepare("SELECT doctor.id as doctor_id , doctor_name , ssd as doctor_ssd , profile_img , front_nationtional_card , back_nationtional_card , graduation_cer , card_id_img FROM doctor,activation_person WHERE doctor.id = activation_person.doctor_id AND activation_person.isactive = 1");
        $get_active_doctor->execute();
        if ($get_active_doctor->rowCount() > 0) {
            $data_doctor_active = $get_active_doctor->fetchAll(PDO::FETCH_ASSOC);
        } else {
        ...
        $get_not_active_doctor = $database->prepare("SELECT doctor.id as doctor_id , doctor_name , ssd as doctor_ssd , profile_img , front_nationtional_card , back_nationtional_card , graduation_cer , card_id_img FROM doctor,activation_person WHERE doctor.id = activation_person.doctor_id AND activation_person.isactive = 0 ");
        $get_not_active_doctor->execute();
        if ($get_not_active_doctor->rowCount() > 0) {
            $data_doctor_not_active = $get_not_active_doctor->fetchAll(PDO::FETCH_ASSOC);
        } else {
        ...
        $get_active_clinic = $database->prepare("SELECT clinic.id as clinic_id , clinic_name , clinic.ser_id as clinic_ser_id , clinic.logo , license_img FROM clinic,activation_place WHERE clinic.id = activation_place.clinic_id AND activation_place.isactive = 1 ");
        $get_active_clinic->execute();
        if ($get_active_clinic->rowCount() > 0) {
            $data_clinic_active = $get_active_clinic->fetchAll(PDO::FETCH_ASSOC);
        } else {
        ...
        $get_not_active_clinic = $database->prepare("SELECT clinic.id as clinic_id , clinic_name , clinic.ser_id as clinic_ser_id , clinic.logo , license_img FROM clinic,activation_place WHERE clinic.id = activation_place.clinic_id AND activation_place.isactive = 0 ");
        $get_not_active_clinic->execute();
        if ($get_not_active_clinic->rowCount() > 0) {
            $data_clinic_not_active = $get_not_active_clinic->fetchAll(PDO::FETCH_ASSOC);
        } else {
        ...
        $get_active_pharmacy = $database->prepare("SELECT pharmacy.id as pharmacy_id , pharmacy_name , pharmacy.ser_id as pharmacy_ser_id , pharmacy.logo , license_img FROM pharmacy,activation_place WHERE pharmacy.id = activation_place.pharmacy_id AND activation_place.isactive = 1 ");
        $get_active_pharmacy->execute();
        if ($get_active_pharmacy->rowCount() > 0) {
            $data_pharmacy_active = $get_active_pharmacy->fetchAll(PDO::FETCH_ASSOC);
        } else {
        ...
        $get_not_active_pharmacy = $database->prepare("SELECT pharmacy.id as pharmacy_id , pharmacy_name , pharmacy.ser_id as pharmacy_ser_id , pharmacy.logo , license_img FROM pharmacy,activation_place WHERE pharmacy.id = activation_place.pharmacy_id AND activation_place.isactive = 0 ");
        $get_not_active_pharmacy->execute();
        if ($get_not_active_pharmacy->rowCount() > 0) {
            $data_pharmacy_not_active = $get_not_active_pharmacy->fetchAll(PDO::FETCH_ASSOC);
        } else {
