...
class Doctor
{
    ...
    {
        $this->db->query("SELECT appointment.id AS appointment_id,patient.id AS patient_id,patient.name,patient.phone_number  FROM  patient,appointment  WHERE appoint_date = :DATE AND appointment.clinic_id = :ID AND appointment.patient_id = patient.id AND appointment.appoint_case = :STATUS ORDER BY appointment.appoint_date");
        $this->db->bind(":ID", $id);
        $this->db->bind(":DATE", $date);
        $this->db->bind(":STATUS", $Status);
        $this->db->execute();
