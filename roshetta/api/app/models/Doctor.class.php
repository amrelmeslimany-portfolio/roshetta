...
class Doctor
{
    ...
    {
        $this->db->query("SELECT * FROM disease WHERE doctor_id = :DO_ID AND clinic_id = :CL_ID AND  patient_id = :PA_ID AND date = :DATE AND name = :NAME AND place = :PLACE ORDER BY id DESC");
        $this->db->bind(":NAME", $data['name']);
        $this->db->bind(":PLACE", $data['place']);
        $this->db->bind(":DATE", $data['date']);
        $this->db->bind(":PA_ID", $data['patient_id']);
