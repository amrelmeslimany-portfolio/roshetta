...
class Admins extends Controller
{
            ...
            if (empty($data['type_user'])) {
                ...
                if (!in_array($data['type_user'], $types)) {
                    ...
                    if (empty($data['user_id'])) {
                        ...
                        if (!$data_user) $data_err['user_id_err'] = 'المعرف غير صحيح';
                    }
                }
            }
            ...
            if (empty($data['type_user'])) {
                ...
                if (!in_array($data['type_user'], $types)) {
                    ...
                    if (empty($data['user_id'])) {
                        ...
                        if (!$this->userModel->getPlace($data['type_user'], $data['user_id'])) $data_err['user_id_err'] = 'المعرف غير صحيح';
                    }
                }
            }
    ...
    public function reply_message_user($id = null)
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($data['message'])) $data_err['message_err'] = 'برجاء إدخال الرد';

            if (empty($data['message_id'])) {
                $data_err['message_id_err'] = 'برجاء إدخال المعرف';
            } else {
                @$data_user = $this->userModel->getPlace('message', $data['message_id']);
                if (!$data_user) $data_err['message_id_err'] = 'المعرف غير صحيح';
            }
                    ...
                    if ($data['type_user_q'] == 'email') {
                        if (empty($data['user_q'])) {
                            ...
                            if (!filter_var($data['user_q'], 274)) {
                                ...
                                if ($this->userModel->getUserEmail($data['user_q'], $data['type_user'])) $data_err['user_q_err'] = 'البريد الإلكترونى موجود من قبل';
                            }
                        }
                    } else {
    ...
    public function edit_profile_user($id = null)
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (!in_array($data['type_user'], $types)) {
                    ...
                    if (empty($data['phone_number'])) {
                        ...
                        if (!filter_var($data['phone_number'], 519) || strlen($data['phone_number']) !== 11) {
                            ...
                            if ($result) {
                                if ($data['user_id'] != $result->id) $data_err['phone_number_err'] = 'رقم الهاتف موجود من قبل';
                            }
                        }
                    }
                }
            ...
            if (empty($data['name'])) $data_err['name_err'] = 'برجاء إدخال الإسم';
            if (empty($data['birth_date'])) $data_err['birth_date_err'] = 'برجاء إختيار تاريخ الميلاد';
            if (empty($data['gender'])) $data_err['gender_err'] = 'برجاء إختيار النوع';
            if ($data['type_user'] == 'doctor') {
                if (empty($data['specialist'])) $data_err['specialist_err'] = 'برجاء إختيار التخصص';
            }
    ...
    public function edit_profile_place($id = null)
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($data['type_user'])) {
                ...
                if (!in_array($data['type_user'], $types)) {
                    ...
                    if (empty($data['place_id'])) {
                        ...
                        if (!$this->userModel->getPlace($data['type_user'], $data['place_id'])) $data_err['place_id_err'] = 'المعرف غير صحيح';
                    }
                    ...
                    if (empty($data['phone_number'])) {
                        ...
                        if (!filter_var($data['phone_number'], 519) || strlen($data['phone_number']) !== 11) {
                            ...
                            @$result = $this->userModel->getUserPhone($data['phone_number'], $data['type_user']);
                            if ($result) {
                                if ($data['place_id'] != $result->id) $data_err['phone_number_err'] = 'رقم الهاتف موجود من قبل';
                            }
                        }
                    }
                }
            ...
            if (empty($data['governorate'])) $data_err['governorate_err'] = 'برجاء إختيار المحافظة';
            if (empty($data['name'])) $data_err['name_err'] = 'برجاء إدخال الإسم';
            if (empty($data['start_working'])) $data_err['start_working_err'] = 'برجاء إختيار ميعاد الفتح';
            if (empty($data['end_working'])) $data_err['end_working_err'] = 'برجاء إختيار ميعاد الغلق';
            if (empty($data['owner'])) $data_err['owner_err'] = 'برجاء إدخال إسم المالك';
            if (empty($data['address'])) $data_err['address_err'] = 'برجاء إدخال العنوان';

            if ($data['type_user'] == 'clinic') {
                if (empty($data['specialist'])) $data_err['specialist_err'] = 'برجاء إختيار التخصص';
                if (empty($data['price'])) $data_err['price_err'] = 'برجاء إدخال السعر';
            }
